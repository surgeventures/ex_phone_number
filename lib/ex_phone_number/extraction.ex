defmodule ExPhoneNumber.Extraction do
  ...
  def maybe_extract_country_code(number, _metadata, _keep_raw_input) when length(number) == 0, do: {false, number, %{country_code: 0}}
  ...
  def maybe_strip_international_prefix_and_normalize(number, _possible_country_idd_prefix) when length(number) == 0, do: {CountryCodeSource.from_default_country, ""}
