defmodule ExPhoneNumber.Formatting do
  ...
  def format(%PhoneNumber{} = phone_number, phone_number_format) when is_atom(phone_number_format) do
    if phone_number.national_number == 0 and not is_nil_or_empty?(phone_number.raw_input) do
      ...
      if PhoneNumberFormats.e164 == phone_number_format do
        prefix_number_with_country_calling_code(phone_number.country_code, phone_number_format, national_significant_number, "")
      else
        if not Metadata.is_valid_country_code?(phone_number.country_code) do
          ...
          prefix_number_with_country_calling_code(phone_number.country_code, phone_number_format, formatted_national_number, formatted_extension)
        end
      end
    end
  end
  ...
  def format_nsn_using_pattern(number, formatting_pattern, phone_number_format, carrier_code) do
    formatted_national_number =
      if PhoneNumberFormats.national == phone_number_format
        and not is_nil_or_empty?(carrier_code) and not is_nil_or_empty?(formatting_pattern.domestic_carrier_code_formatting_rule) do
        carrier_code_rule = Regex.replace(Patterns.cc_pattern, formatting_pattern.domestic_carrier_code_formatting_rule, carrier_code)
        number_rule = Regex.replace(Patterns.first_group_pattern, formatting_pattern.format, carrier_code_rule)
        Regex.replace(formatting_pattern.pattern, number, number_rule)
      else
        if PhoneNumberFormats.national == phone_number_format and not is_nil_or_empty?(formatting_pattern.national_prefix_formatting_rule) do
          number_rule = Regex.replace(Patterns.first_group_pattern, formatting_pattern.format, formatting_pattern.national_prefix_formatting_rule, global: false)
          Regex.replace(formatting_pattern.pattern, number, number_rule)
        else
  ...
  def prefix_number_with_country_calling_code(country_code, phone_number_format, formatted_national_number, formatted_extension) do
    country_code_string = Integer.to_string(country_code)
    cond do
      PhoneNumberFormats.e164          == phone_number_format -> Values.plus_sign <> country_code_string <> formatted_national_number <> formatted_extension
      PhoneNumberFormats.international == phone_number_format -> Values.plus_sign <> country_code_string <> " " <> formatted_national_number <> formatted_extension
      PhoneNumberFormats.rfc3966       == phone_number_format -> Values.rfc3966_prefix <> Values.plus_sign <> country_code_string <> "-" <> formatted_national_number <> formatted_extension
      PhoneNumberFormats.national      == phone_number_format -> formatted_national_number <> formatted_extension
      true                                                    -> formatted_national_number <> formatted_extension
    end
  end
end
